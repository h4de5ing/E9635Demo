...
public class J1939Utils {
  ...
  /*          if (i == CurrentType) {
     ...
     *
     * @param id
     * @return CAN 2.0A 标准帧为11个字节 包含信息和数据两部分，前3个字节为信息部分。
     * 字节1：第7位FF表示帧格式，在标准帧中FF=0，在扩展帧中FF=0
     * 第6位RTR表示帧类型   RTR=0表示数据帧 RTR=1 表示远程帧
    ...
    public static void saveDataInfo2File(String string) {
        DateFormat formatter = new SimpleDateFormat("yyyyMMddHHmmss");
        try {
            String time = formatter.format(new Date());
            String fileName = "acc.log";//"data-" + filename + "-" + time + ".log";
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                String path = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "e9631_log" + File.separator;
                ...
                fos.write("----------\n".getBytes());
                fos.write((time + ":" + string).getBytes());
                fos.write("\n----------".getBytes());
                fos.close();
            }
        } catch (Exception e) {
